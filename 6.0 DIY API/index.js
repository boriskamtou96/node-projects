...
app.delete('/jokes/:id', (req, res) => {
  ...
  if (existingJoke) {
    const searchIndex = jokes.findIndex((joke) => joke.id === existingJoke.id);
    jokes.splice(searchIndex, 1);
    res.status(200).json('Joke delete successfully!!!');
  } else {
    res.status(400).json({ error: `Joke with id ${id} not found!!!` });
  }
});
...
app.delete('/all', (req, res) => {
  ...
  if (userkey === masterKey) {
    ...
    res
      .status(404)
      .json({ error: `You are not authorized to perfom this actions.` });
  }
});
